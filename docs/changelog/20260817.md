...
- **`/data` is encrypted** — file-based encryption, with a lock screen if you want one. ⚠️ Requires erasing `/data`. [Details](#encryption)
- **The device comes up landscape**, from the setup wizard onwards, with touch and auto-rotate following it. [Details](#landscape-by-default)
- **Updates install themselves** — pick, download, tap install, and the device does the rest. [Details](#updates-install-themselves)
- **An update refuses to half-install itself** if the vendor partition is older than the build expects. [Details](#an-update-refuses-to-half-install-itself)
- **A monitor plugged into mini-HDMI now works on its own**: it comes up without being asked, keeps going in fullscreen apps, carries sound, draws smoothly, and survives sleep. [Details](#mini-hdmi)
- **Display out is no longer marked experimental**, and its page says what actually remains rough. [Details](#display-out)
- **Vulkan advertises the extensions modern engines check for**, so apps stop silently falling back to OpenGL. Still Vulkan 1.0. [Details](#vulkan-newer-extensions)
- **Hardware-info apps see the real Vulkan driver identity** instead of "unknown" and zeroed UUIDs. [Details](#vulkan-driver-identity)
- **The shader cache moved into each app's own storage**, so it is counted, cleared and reclaimed like any other app data. [Details](#vulkan-shader-cache)
- **SELinux enforcing works**, with a device policy and a domain per service. Still not the default. [Details](#selinux)
- **TWRP keeps its settings and logs on the microSD card**, not on `/data`, which it cannot decrypt. ⚠️ Keep a card in the device. [Details](#recovery)
- **The device no longer claims sensors it does not have** — the gyroscope, compass, light and proximity sensors were never fitted. [Details](#sensors)
- **Deleting a large folder from the SD card works**, and finishes in seconds rather than failing silently. [Details](#deleting-a-large-folder-from-the-sd-card)
- **Monitoring apps can read the CPU clusters, the battery current and the GPU** through the standard kernel interfaces. [Details](#kernel-hardware-visible-to-monitoring-apps)
- **The "GPD XD+" menu buttons work**, and the menu is now its own app rather than a modified Settings. [Details](#the-gpd-xd-menu)
- **Fewer false alarms in the logs** — three display watchdogs removed or scaled back. [Details](#fewer-false-alarms-in-the-logs)
...
Everything about mirroring to a monitor, in one place. It comes up by itself, keeps going in fullscreen apps, carries sound, and draws smoothly.
