...
**Second release.** 2026-08-17. [Download](https://github.com/Keyaku/gpd-xdplus-customrom/releases/tag/20260817) · one zip, `lineage-18.1-20260817-UNOFFICIAL-xdplus.zip`, which writes `system`, `boot` and `vendor`.
...
⚠️ **Read [Installing](../INSTALL.md) before updating.** This is not like a normal update: it writes the vendor partition, which turns on encryption, and turning on encryption requires erasing `/data`. Coming from 20260804 means a wipe, not an in-place update.
...
⚠️ **This lives in the vendor image**, which the release zip writes along with system and boot.
...
- **The build now checks the vendor partition before it writes anything**, and refuses rather than applying half a feature. The published package sidesteps the check by carrying vendor itself; the check still guards a build installed without it.
...
⚠️ **The vendor-software half of that lives in the vendor image**, which the release zip writes. The filter alone is enough on a device whose vendor partition is not updated.
